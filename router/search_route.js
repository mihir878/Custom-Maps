...
const entertainmentMap = require('../public/js/entertainment');
require('dotenv').config();

function getPlacesTypes(lat,lng,rad)
    {
        let API_KEY_GLOBAL = process.env.API_KEY;
...
router.get('/',(req,res)=>{
    ...
    arr
        .then((allPlaces)=>{
            ...
            for(let i=0;i<allPlaces.length;i++)
                {
                    ...
                    score = score + (finalMap.get(allPlaces[i][r])===undefined?0:finalMap.get(allPlaces[i][r]));
                    results.push(score);
                }
            ...
            res.render('map',{src: req.query.src,dest: req.query.dest,safety: req.query.safety,entertainment: req.query.entertainment,index: index-1,API_KEY: API_KEY_GLOBAL});
        })
        .catch((error)=>{
            console.log(error);
